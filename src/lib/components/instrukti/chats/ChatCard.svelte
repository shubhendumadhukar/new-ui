...
<button class="flex flex-col items-start gap-2 rounded-lg border p-3 text-left text-sm transition-all hover:bg-accent {$mailStore.selected === mail.id ? 'bg-muted' : ''}" on:click={() => mailStore.setMail(mail.id)}>
